...
The reproducibility layer hardens the canonical Evaluation framework without introducing a second evaluation lifecycle. `EvaluationCase`, `EvaluationSuite`, `EvaluationRun`, `EvaluationAttempt`, `EvaluationResult`, aggregation and regression remain owned by Evaluation; `EvalManifest` is immutable reproducibility evidence bound to one `EvaluationRun`.
...
- `directly_comparable` / `comparable_with_warnings`: the normal canonical regression engine may proceed.
...
This reproducibility hardening does not change the ownership of CI or performance methodology. Performance benchmarks remain independently owned; the manifest only classifies environment/resource drift when performance-sensitive comparison is requested.
