...
- may create many Tasks
- may be associated with one or more Results

### Project

Logical workspace/ownership boundary grouping Goals, Tasks, Agents, Artifacts and related configuration. Project does not imply a filesystem workspace or Forge project.
...
- one Goal
- one Project
- zero or more Plans over its lifetime
- zero or more Runs
- zero or more Artifacts and Results
...
- `draft -> cancelled`
- `ready -> cancelled`
- `failed -> ready` only through an explicit retry/reset command in later lifecycle work

`running` describes canonical work in progress, not the state name of any external backend.

### Plan

A versioned decomposition of a Task into ordered and/or dependency-linked Steps. A Task may receive multiple Plan versions as reasoning changes. Plans are immutable once activated; revisions create a new Plan version.

### Step / Subtask

A Step is an executable or delegatable unit belonging to a Plan. Nested work is represented by `parent_step_id` rather than a different canonical entity type. The term Subtask is therefore a relationship/role of Step, not a separate storage identity.
...
Run contains canonical execution metadata and may reference external orchestrator/executor/model identifiers through `external_refs`.

Initial Run states:

`queued -> running -> {succeeded | failed | cancelled | timed_out}`

A Run must identify its subject (`task` or `step`) and attempt number.
...
An Agent may have model assignments and tool/capability requirements. Runtime sessions/instances are execution concerns and should not replace the stable Agent identity.
...
Immutable or version-addressable output/input produced, consumed or attached during work. Artifacts may represent files, structured data, code patches, reports, images or other payloads. The payload backend is delegated to the File/Artifact provider boundary; canonical metadata remains platform-owned.

Artifact records include provenance links to producing/consuming Runs where known.

### Result

Semantic outcome of a Task/Run. A Result summarizes completion and references zero or more Artifacts. Result is distinct from raw execution output.
...
Records a requested human/policy decision associated with a Task, Run, Step, Tool invocation or other governed action. Authorization and policy evaluation are deferred to issue #15.
...
Canonical compute/resource endpoint participating in the platform. A Node can represent a desktop, notebook, workstation, home server, VPS, cloud VM, GPU host, NAS-capable worker host or other device. Provider/hardware class is metadata, not identity.
...
Canonical tool definition exposed to agents/tasks. Concrete native, MCP, HTTP or plugin implementations live behind adapters.

### Capability

A discoverable functional/property descriptor used for matching requirements to tools, workers, nodes, models or agents. Capability identifiers must be implementation-neutral where possible.

### Model Assignment

A versioned association between a subject (Agent, Task, Step or policy scope) and model requirements/provider selection. Canonical assignments must not require a specific provider. Concrete provider/model identifiers are configuration values behind the model-provider boundary defined later.
...
- Task terminal state cannot be inferred from one backend-specific state string; platform lifecycle rules decide it.
...
Adapters receive and return canonical references/contracts. Backend-specific state belongs in adapter-owned translation structures or `external_refs`. Examples of prohibited leakage into canonical contracts include Forge database row types, Hermes session objects, Temporal workflow handles, LiteLLM response classes or MCP transport objects.

## Deferred decisions

This issue deliberately does not select:

- database technology
- event store implementation
- orchestrator/executor APIs
- model gateway
- memory/vector backend
- file/object storage product
- scheduler implementation
- authorization engine

Those decisions belong to later numbered issues and must conform to this domain model rather than reshape it implicitly.
