...
The machine-readable source/revision/invariant manifest lives at `tests/evidence/issue_862/storage_backends.json`, with repository tests guarding the architecture boundary. Remaining work is runtime evidence: candidate services, identical S3 contract probes, restart/failure/backup/upgrade campaigns, and measured VPS resource results.
