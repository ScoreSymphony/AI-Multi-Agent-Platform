...
The Agent/Team domain is the canonical owner of Agent, Team and AgentRun state.
...
`agent-runs` exposes canonical runtime records when the Agent runtime is composed.
...
If the Agent domain is not registered in the target Control Plane, the CLI fails at extension discovery and performs no repository/runtime/backend fallback.
