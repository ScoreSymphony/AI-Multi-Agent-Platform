...
Deleting a workflow file does not immediately remove its old runs from the Actions sidebar. `repository-maintenance.yml` therefore reads the actual `.github/workflows` directory from the exact commit being maintained and treats those files as the source of truth. This is intentionally stricter than relying on the Actions workflow registry alone, whose entries can outlive deleted workflow files. A run is eligible for cleanup only when its recorded path starts with `.github/workflows/` and that exact file is absent from the repository directory.

Routine cleanup is deliberately bounded. It first identifies registered workflow paths that no longer have a matching file in `.github/workflows`, then inspects at most 10 pages of 100 runs for each orphaned workflow ID. It also checks at most 10 pages of recent completed repository runs as a fallback discovery surface. Repeated maintenance runs therefore make steady progress without paginating the repository's complete Actions history on every execution.
