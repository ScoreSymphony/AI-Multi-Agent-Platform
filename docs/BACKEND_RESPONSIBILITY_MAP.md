...
## Cohort 9 — Coordination registration and graph validation extraction

The ninth implementation cohort starts decomposing the durable coordination hotspot while retaining `DurablePlanStepCoordinator` as the orchestration façade:

- `coordination/registration.py` owns Step-graph validation, canonical Task/Plan identity checks, retry-policy reference validation and creation of the initial durable coordination records.
- `DurablePlanStepCoordinator.register_plan` delegates registration mechanics to `CoordinationRegistration`, then retains façade-level telemetry, initial progression and projection behavior.
- `_validate_graph` remains as a thin compatibility shim because the superseded-Plan retirement layer currently performs destructive-transition prevalidation through that inherited private seam.
- `CoordinationRegistration` depends only on `CoordinatorRepository` plus a narrow canonical Task reader protocol and never imports the coordinator façade.
- Kernel ownership of canonical Task truth and repository ownership of durable coordination state remain unchanged.

Architecture guards require `register_plan` and `_validate_graph` to keep delegating to the focused component and prevent `registration.py` from depending back on `coordination.service`.
...
1. Extract graph validation/registration. *(implemented)*
