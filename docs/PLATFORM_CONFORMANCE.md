...
| ARCH — architecture invariants | canonical/backend isolation, northbound Python client backend isolation, backend-private public-type guard and platform-owned infrastructure-neutral Task/Run identity | #46 |
...
`tests/contract/portability/test_architecture_invariants.py` currently automates six platform-boundary invariant families plus a focused self-test of the annotation guard:
...
2. CLI, Chat/conversation and Terminal Python client-domain code must not import platform adapter implementations or Hermes/Forge/LiteLLM/MCP runtime packages directly;
...
5. host/process/HA-leader metadata and Worker relocation must not rewrite platform-owned canonical Task/Run identity;
6. optional backend packages such as LiteLLM/MCP/provider SDKs must not become mandatory platform runtime dependencies.

The Python client-domain check complements the maintained Control Plane parity scenarios; the Web client remains covered by its own API-facing integration/acceptance evidence rather than by this Python import scanner.
