...
| SEC-BND | production security-boundary matrix across every claimed side-effect surface, including authn/authz/approval/scope/secret/audit/fail-closed/error/provider-neutral-authority regressions | #1233 / canonical security owners |

All four `REL-*` lifecycle checks are `required=true` whenever `--profile release` is selected. The `SEC-BND` production security-boundary scenario is also required for the release profile. A release report therefore cannot be `compatible` merely because these capabilities have unit tests elsewhere; the representative acceptance commands must pass inside the same platform release claim.

`SEC-BND` is the maintained production security-boundary aggregate for #1233. Its machine-readable surface inventory lives in [`conformance/security/production_boundary_matrix.toml`](../conformance/security/production_boundary_matrix.toml) and is validated/executed by [`scripts/ci/security_boundary_conformance.py`](../scripts/ci/security_boundary_conformance.py). The matrix reuses subsystem-owned tests and fails if claimed surfaces lose canonical authentication, authorization, exact approval, scope isolation, SecretReference/redaction, auditability, failure/cancellation, fail-closed behavior, safe public errors or provider-neutral authority. See [`security/PRODUCTION_SECURITY_BOUNDARY_CONFORMANCE.md`](security/PRODUCTION_SECURITY_BOUNDARY_CONFORMANCE.md).
