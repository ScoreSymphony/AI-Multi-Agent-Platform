...
- Files/Artifacts — canonical `files` metadata inventory/detail from #13 plus read-only Artifact, Result, Plan and Step references. File scope, state, content type, size, checksum, owner/Project scope and Artifact relationships are read only through `/api/v1/files`; raw bytes, storage paths and provider-private storage identity are intentionally not inferred;
