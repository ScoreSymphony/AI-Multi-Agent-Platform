...
- Tasks — canonical Task management, including the practical Task-management queue/filter/sort/metadata/dependency/bulk-management surface already exposed by the Control Plane;
- Runs — canonical Run list/detail, including the canonical Run-to-Workspace binding and direct Workspace navigation when a Run records `workspace_id`, snapshot and checksum provenance;
- Agents — canonical Agent definitions with immutable current revision, role, model/capability/data-access policy metadata, read-only AgentRun evidence and assigned canonical Task projections with direct Plan/Run links; create/edit/clone and manifest-advertised owner-safe delete are available, with explicit confirmation before delete;
- Agent Teams — canonical Team definitions with immutable current revision, exact pinned member Agent revisions, delegation relationships, shared capabilities and runtime limits, plus assigned canonical Task projections with direct Plan/Run links; create/edit and manifest-advertised owner-safe delete are available, with explicit confirmation before delete;
- Verification — canonical verification queue/detail and review actions from Verification, including independently paginated pending-review, completion-requirement and history inventories, Task/Run/reference evidence links and manifest-gated availability;
- Files/Artifacts — canonical `files` metadata inventory/detail from the File domain plus read-only Artifact, Result, Plan and Step overview/detail routes; Step detail also exposes Runs filtered by the canonical Run subject projection. File scope, state, content type, size, checksum, owner/Project scope and Artifact relationships are read only through `/api/v1/files`; raw bytes, storage paths and provider-private storage identity are intentionally not inferred;
- Memory — canonical scoped `memory` content from the Memory domain with scope-aware list/search/detail, explicit create, short-term promotion, supersession-based update, exact expiry and delete. Project-scoped Memory uses `workspace` plus the canonical Project ID. Origin, retention, provenance, classification and supersession links are preserved; Chat/Event/Task history is never silently reclassified as Memory;
- Knowledge — canonical `knowledge` source inventory/detail from the Knowledge domain plus query-scoped `knowledge-results`. The UI supports explicit source registration, metadata update, ingestion, revisioned re-index, detach/delete tombstones and authorized retrieval with canonical source/revision citations. Retrieval rows are not treated as durable resources, and vector/index/backend-private identifiers are never exposed as canonical identity;
...
- Integrations — canonical `connector-definitions` and `connections` from the Connector domain, including safe Connector metadata, Connection create/enable/disable/remove/health lifecycle and explicit `incremental|resync|rebuild` synchronization. Secret material is never accepted or rendered by this surface, and external connector actions remain behind the canonical Capability pipeline rather than a browser-side `connector.invoke` bypass;
- Repositories — canonical `repositories` from the Repository domain with provider-neutral inventory/detail, status/refs/commit/diff inspection, managed local attach, Connection/provider discovery with optional attach, fetch, branch creation, checkout, commit, push and detach. Capability metadata gates provider-supported Git mutations, high-impact actions use explicit confirmation, detach removes only the platform binding, and the browser never receives provider credentials, arbitrary host paths or provider SDK identities;
...
- Compute — canonical `nodes`, `workers` and `worker-jobs` from the Node/Worker domain, including resource/accelerator availability, heartbeats, capabilities, placement requirements and dispatch evidence. Administrative UI is limited to the exact northbound `node.drain|undrain|maintenance-enable|maintenance-disable` and `worker.drain|undrain` commands; the browser never calls Worker transports, registries or schedulers directly;
- Applications — canonical `applications`, `application-instances` and `application-logs` from the Application Adapter owner. The Web surface exposes inventory/detail, lifecycle/health/log inspection, start/stop/restart/reconcile/remove and manifest-typed mutable configuration through `application.configure`. Removal requires explicit confirmation; configuration never exposes plaintext secrets or runtime-private process/container identity;
- Terminal/Sessions — canonical Terminal session UI and Control Plane streaming gateway from the Terminal domain, without exposing backend process/session handles as frontend identity;
- Automations — canonical `automations` and `automation-deliveries` management from the Automation domain, including create/update, pause/resume/disable, re-enable of disabled state through canonical resume, recovery of `invalid` state through `automation.revalidate`, manual test delivery, delivery history and failed-delivery retry. Disable requires explicit confirmation. V1 exposes no `automation.delete` command, so the browser does not invent Automation deletion. Webhook ingestion, platform-event injection, administrative invalidation and scheduler evaluation remain system/integration paths rather than ordinary browser buttons;
- Templates — canonical `templates` and `template-instances` from the Template domain. The browser supports list/detail, immutable revision history, editable new draft revisions, publish, clone/fork, create-from-existing Agent, Agent Team, Workflow, Capability Assignment, Model Routing Profile, Automation, Project and Workspace structures, server-resolved preview and apply. Template instances preserve every generated resource reference as canonical type + ID in canonical instance data. Workflow, Capability Assignment and Model Routing Profile instance references now link to manifest-gated read-only owner-domain detail routes backed by the canonical `workflows`, `capability-assignments` and `model-routing-profiles` Control Plane collections; these views do not create Template-private lifecycle or persistence. Other generated resource types are linked only when a canonical browser detail route exists, and unknown/future types remain visible by stable ID without a fabricated provider/private/backend route. Preview renders dependency, capability, plugin, connector, model-policy, Workspace, permission and privileged-capability implications before apply. Published revisions and prior instances are never rewritten by a later edit/apply. Template configuration remains canonical intent only: plaintext secrets, runtime-private state and provider/orchestrator-private identities are rejected by the server rather than reconstructed or persisted by the browser;
- Import/Export — canonical `portability-packages`, `portability-import-previews` and `portability-import-reports` from the Portability domain. The browser can export explicit canonical resource references, validate incoming portable package JSON, create and inspect the server-owned import preview and execute only an exact ready Preview after explicit confirmation. ID mapping and mutation order remain server-owned; the Web client cannot submit or rewrite them, and secret/runtime exclusions remain enforced by the canonical portability boundary;
- Organizations/Collaboration — canonical Organizations, Teams, Memberships, invitations, ownership and sharing from the Organization domain. Membership removal, invitation revocation and share revocation require explicit browser confirmation while authorization and durable collaboration state remain server-owned;
- Plugins — canonical `plugins` and optional `plugin-candidates` from the Plugin domain. Installed Plugin inventory/detail exposes state, compatibility, health, permissions, extensions, manifest and provenance. Candidate inspection pins install/update actions to the inspected manifest digest; lifecycle uses only `plugin.install|configure|enable|disable|refresh-health|validate-update|remove`. Stored configuration is intentionally not reconstructed after `plugin.configure`, and Candidate discovery degrades independently when no northbound PluginCatalog is composed;
...
- Notifications — cursor-paginated canonical notification inventory plus mark-read/mark-all-read, acknowledge, dismiss, archive, delivery retry and user attention preferences from Notifications, gated by the advertised `notifications` resource; notification source lifecycles remain owned by their canonical domains;
- Settings/Authentication — browser login, current canonical identity, browser-session inventory, renewal, targeted session revocation and logout, plus short-lived Mobile pairing and paired-device list/rename/revoke/revoke-all management through canonical Authentication routes; the HttpOnly session secret and Mobile device bearer credential remain opaque to frontend code;
- Events/Observability — Task-scoped timeline and available backend-neutral observability information. Recent Tasks remain convenient to select, while exact `/events/:taskId` and `/observability/:taskId` routes plus the canonical Task-ID selector keep older Tasks reachable without widening telemetry authority; both timeline and trace rows retain opaque-cursor continuation. Trace presentation resolves canonical Task Run records so Run-linked Workspace IDs can navigate to the maintained Workspace detail surface without making Observability a Workspace authority.
- Usage & Limits — canonical usage records, aggregates and budgets exposed by the Control Plane. Records, aggregates and budgets keep independent opaque-cursor pagination; independent collection failures retain canonical permission/approval/unavailable/backend error semantics so healthy accounting collections remain usable without flattening failures into generic text.
...
The shell composes the browser-session boundary in front of the shared `ControlPlaneClient`. All requests continue to use `credentials: include`; the opaque session secret exists only in the server-issued `HttpOnly` cookie and is never read or persisted by frontend code.

`POST /api/v1/auth/login` and `POST /api/v1/auth/session:renew` return the separate browser CSRF token required by the Authentication domain. The frontend retains only that CSRF token in same-origin `localStorage`; it is not a bearer credential or session secret. Before every unsafe cookie-authenticated request, the shared session-aware fetch boundary re-reads the current stored value and injects `X-CSRF-Token`. This keeps concurrent tabs aligned when session renewal rotates the CSRF token while allowing the HttpOnly authentication cookie to remain opaque. Existing Task, Model, Workspace, Terminal, Automation, Approval, Template, Portability, Verification, Notification, Evaluation, Compute, Integration, Plugin, Memory, Knowledge, Organization and Conversation mutations therefore inherit Authentication CSRF protection without duplicating security logic in individual pages. Requests carrying an explicit Bearer `Authorization` header do not receive the browser CSRF header.

The Settings surface consumes only canonical Authentication routes for login, `auth/me`, session enumeration, renewal, targeted session revocation and logout. It also exposes the maintained Mobile companion pairing/device-management flow: create or cancel a short-lived pairing challenge, list paired devices, rename a device and revoke one or all device credentials. Pairing consumption and durable device-secret storage remain client/server Authentication responsibilities; the browser never receives or stores the resulting device bearer credential. First-user bootstrap is handled by the dedicated browser-first initialization flow, while password recovery and general credential/PAT administration retain their separate operator/authorization semantics.

Authentication establishes identity only. Permission hooks remain advisory presentation hints, and buttons are never proof of authorization: the Control Plane and Authorization remain authoritative. Canonical unauthenticated and authorization-denied responses are presented separately. Approval-required authorization outcomes are surfaced with their canonical approval reference when the Control Plane returns one; the frontend does not manufacture or bypass approval state.

The Approval surface consumes the safe shared northbound Authorization decision contract. Reads remain on the canonical `approvals` collection; mutations use only `POST /api/v1/commands/approval.approve` and `POST /api/v1/commands/approval.deny` through the session-aware `BrowserSessionClient` boundary. Each decision forwards the exact canonical Approval ID and the already-inspected immutable `requested_action_digest`, generates/forwards an idempotency key and correlation ID, and may include only an optional decision comment. The browser never reconstructs a proposed action payload or calls `ApprovalService.decide()`, `_decide_authorized()` or any private security/database primitive. Approver authorization, actor identity, exact-action binding, expiry, pending-state validation, policy and audit semantics remain server-authoritative. The UI requires explicit review/confirmation of the displayed canonical context, but hidden/disabled controls are never treated as authorization enforcement.
...
Automation ownership is also server-derived. Creation uses the authenticated canonical actor context rather than client-supplied identity. Configuration mutations use the exact Automation command names and remain authorization-gated server-side. Webhook configuration accepts only the canonical verification reference; embedded webhook secrets are not a browser configuration field. Delivery payloads are not rendered by default in history views.
...
- browser-session login/CSRF behavior, including CSRF propagation to existing Control Plane mutations, cross-tab rotation synchronization, Bearer separation and logout cleanup;
- canonical Workspace and reference-resource contracts;
- canonical File metadata collection/detail forwarding through `/api/v1/files`, including Project filters and explicit absence of browser-invented raw-byte/provider routes;
...
- canonical Repository collection/detail forwarding plus `repository.local.attach|discover|fetch|branch.create|checkout|commit|push|detach` routing, including managed-path isolation, capability-gated mutation controls, approval/idempotency propagation and no provider-private fallback;
...
- canonical Application definition/instance/log reads, lifecycle commands and manifest-typed `application.configure` editing, including removal confirmation and continued SecretReference/runtime-private isolation;
...
- canonical Automation create/update/lifecycle/manual-test/retry command routing with idempotency, including disabled-to-enabled resume and administrative revalidation of `invalid` Automation state without inventing delete;
- manifest-gated Chat, Automation, Approval, Verification, Notification, Template, Plugin, Files, Memory and multi-resource Integration/Evaluation/Compute/Knowledge/Import-Export routing;
- a repository-wide navigation closure guard proving every discoverable primary sidebar route resolves to a maintained route instead of the generic unavailable fallback, plus deep-link parent-navigation coverage;
...
| --- | --- | --- | --- |
| `react`, `react-dom` | 19.3.0 | web rendering runtime | MIT, `facebook/react` |
| `vite` | 8.3.0 | dev server / production build | MIT, `vitejs/vite` |
| `@vitejs/plugin-react` | 6.1.1 | React transform / Fast Refresh | MIT, `vitejs/vite-plugin-react` |
| `typescript` | 7.0.2 | static type checking | Apache-2.0, Microsoft TypeScript |
| `vitest` | 5.0.0 | frontend unit tests | MIT; bundled license families are recorded by the package and Vite build license output |
| `@types/react`, `@types/react-dom` | 19.3.0 / 19.3.0 | development typings | MIT, DefinitelyTyped |
...
## API-gated product scope

The browser activates a product domain only when the current Control Plane manifest advertises the canonical resources and commands required by that surface. An owning backend implementation by itself is not enough, and the browser never falls back to private repositories, provider SDKs or runtime services.

The maintained shell currently integrates the product areas listed in `frontend/README.md`, including core Task/Run/Project/Workspace flows; Agents and Agent Teams; Verification and Approvals; Files/Artifacts, Memory, Knowledge and Search; Models/Providers, Tools/Capabilities and Integrations/Connectors; Templates, Registry/Marketplace and portability; Compute Nodes/Workers; and the operator-facing observability, notification, governance and configuration surfaces exposed by the active manifest.

Optional or advanced resources remain manifest-gated. A deployment that does not advertise the required canonical resource receives the shared unavailable/degraded presentation rather than a speculative client implementation. Experimental surfaces retain their explicit maturity signaling under the repository-wide feature-classification policy.

Generated or related resources link only to maintained canonical browser routes. Unknown or future resource types remain inspectable by stable identity without fabricating provider-private navigation. This keeps the Web UI a replaceable northbound client of the same versioned Control Plane used by the CLI and external clients.
