...
- Verification — canonical verification queue/detail and review actions from #86, including independently paginated pending-review, completion-requirement and history inventories, Task/Run/reference evidence links and manifest-gated availability;
- Files/Artifacts — canonical `files` metadata inventory/detail from #13 plus read-only Artifact, Result, Plan and Step overview/detail routes; Step detail also exposes Runs filtered by the canonical Run subject projection. File scope, state, content type, size, checksum, owner/Project scope and Artifact relationships are read only through `/api/v1/files`; raw bytes, storage paths and provider-private storage identity are intentionally not inferred;
...
- Import/Export — canonical `portability-packages`, `portability-import-previews` and `portability-import-reports` from #79. The browser can export explicit canonical resource references, validate incoming portable package JSON, create and inspect the server-owned import preview and execute only an exact ready Preview after explicit confirmation. ID mapping and mutation order remain server-owned; the Web client cannot submit or rewrite them, and secret/runtime exclusions remain enforced by the canonical portability boundary;
- Organizations/Collaboration — canonical Organizations, Teams, Memberships, invitations, ownership and sharing from #87. Membership removal, invitation revocation and share revocation require explicit browser confirmation while authorization and durable collaboration state remain server-owned;
...
- Notifications — cursor-paginated canonical notification inventory plus mark-read/mark-all-read, acknowledge, dismiss, archive, delivery retry and user attention preferences from #75, gated by the advertised `notifications` resource; notification source lifecycles remain owned by their canonical domains;
