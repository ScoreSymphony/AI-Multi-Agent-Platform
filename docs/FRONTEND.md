...
- Events/Observability — Task-scoped timeline and available backend-neutral observability information. Recent Tasks remain convenient to select, while exact `/events/:taskId` and `/observability/:taskId` routes plus the canonical Task-ID selector keep older Tasks reachable without widening telemetry authority; trace rows retain their own opaque-cursor continuation.
