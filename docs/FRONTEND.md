...
- Settings/Authentication — browser login, current canonical identity, browser-session inventory, renewal, targeted session revocation and logout; the HttpOnly session secret remains opaque to frontend code;
- Events/Observability — Task-scoped timeline and available backend-neutral observability information. Recent Tasks remain convenient to select, while exact `/events/:taskId` and `/observability/:taskId` routes plus the canonical Task-ID selector keep older Tasks reachable without widening telemetry authority; both timeline and trace rows retain opaque-cursor continuation.
...
The Settings surface consumes only canonical Authentication routes for login, `auth/me`, session enumeration, renewal, targeted session revocation and logout. First-user bootstrap, password recovery and credential/PAT administration are intentionally not inferred as ordinary browser workflows merely because backend hooks exist; they retain their separate operator/authorization semantics.
