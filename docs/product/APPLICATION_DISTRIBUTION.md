...
Secret-backed builds remain an explicit asymmetry: the local/reference backend implements late `SecretReference` resolution, while the distributed backend currently rejects non-empty `secret_environment` before dispatch until scoped Worker secret delivery is implemented. That remaining limitation must not be described as a lack of remote Worker build dispatch in general.
