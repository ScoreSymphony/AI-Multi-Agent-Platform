...
For `EventProvider.publish`, publishing the same canonical `Event.id` repeatedly is idempotent. An event transport/store must not create a second canonical event for an identifier that was already accepted.
