...
Capability assignment policies are durable canonical resources for reusable capability/tool assignment intent. They deliberately sit **next to** the Capability Registry instead of replacing it.
...
The Capability Registry remains authoritative for capability identity, versions, features,
...
The canonical authorization boundary remains authoritative for create, revise, read and list
...
validity still belong to the Capability and Authorization domains at invocation time.
...
The `CapabilityAssignmentTemplateHandler` instantiates `capability_assignment` Templates by
...
preserved, and the assignment service re-enters the shared `AuthorizationGate` before durable
creation.
...
Portability codecs/remapping can wrap the same canonical repository/service without redefining assignment identity or granting authority merely because an asset was imported. Provider handles,
