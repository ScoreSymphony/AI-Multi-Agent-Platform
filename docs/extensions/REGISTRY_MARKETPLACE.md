...
| --- | --- | --- | --- | --- | --- |
| Agent | `AgentService` / canonical Agent repository | yes, importing ordinary immutable Agent revisions | yes, exactly one revision at a time without changing ownership scope | yes, through `AgentService.delete_agent()` | current canonical Agent revision; no `AgentRun` is created |
| Agent Team | `AgentService` / canonical Agent repository | yes, importing ordinary Team revisions after canonical member validation | yes, exactly one Team revision at a time | yes, through `AgentService.delete_team()` | current canonical Team revision; no runtime team instance is created |
| Orchestrator | #20 Plugin lifecycle, then `ORCHESTRATOR` binder into `OrchestratorRegistry` on explicit plugin enable | yes | yes, through the Plugin owner | yes, through the Plugin owner | Plugin package state plus normal Orchestrator inventory after enable |
| Executor | #20 Plugin lifecycle, then `EXECUTOR` binder into `ExecutorRegistry` on explicit plugin enable | yes | yes, through the Plugin owner | yes, through the Plugin owner | Plugin package state plus normal Executor inventory after enable |
| Model Provider | #20 Plugin lifecycle, then `MODEL_PROVIDER` binder into `ModelRegistry` on explicit plugin enable | yes | yes, through the Plugin owner | yes, through the Plugin owner | provider implementation only; configured models/credentials remain separate owner resources |
| Capability Provider | #20 Plugin lifecycle, then `CAPABILITY_PROVIDER` binder into `CapabilityRegistry` on plugin enable | yes | yes, through the Plugin owner | yes, through the Plugin owner | canonical Plugin/provider state |
...
and supplies durable installation state plus `PlatformRegistryValidationContextResolver`. The resolver derives platform version, host OS/architecture, available application runtimes, installed Registry dependencies, capabilities, plugins, **ConnectorDefinition IDs**, models and grantable permissions from live server-side platform state rather than accepting those claims from a client. Connector and runtime requirements therefore use the same canonical inventories as the rest of the single-node platform. Optional signature keys add the reference HMAC verifier at the same composition boundary.
