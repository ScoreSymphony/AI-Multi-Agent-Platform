...
| Skill | `SkillService` / Skill repository | yes, canonical revision 1 | yes, exactly one Skill revision at a time | yes, using the repository's existing historical-reference checks | current canonical Skill revision |
