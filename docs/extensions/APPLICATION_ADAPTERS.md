...
Under the unified Marketplace contract from #1174, **manifest-backed** Application items use `ApplicationMarketplaceKindHandler` only as a thin distribution adapter. Install delegates into the existing `ApplicationLifecycleService`, `ApplicationRepository` and `ApplicationRuntimeRegistry`; Marketplace does not provision, start, stop, reconcile or persist Application runtime state itself. Installation is available only when the catalog manifest can form a complete canonical `ApplicationInstallRequest` without unresolved configuration, secret or volume bindings. Marketplace artifact-version update remains fail-closed because the #1173 owner exposes configuration/restart operations but no canonical artifact-version migration. Status and uninstall likewise resolve the canonical Application instance and delegate to the #1173 owner.
