...
Under the unified Marketplace contract, **manifest-backed** Application items use `ApplicationMarketplaceKindHandler` only as a thin distribution adapter. Install delegates into the existing `ApplicationLifecycleService`, `ApplicationRepository` and `ApplicationRuntimeRegistry`; Marketplace does not provision, start, stop, reconcile or persist Application runtime state itself. Installation is available only when the catalog manifest can form a complete canonical `ApplicationInstallRequest` without unresolved configuration, secret or volume bindings. Marketplace artifact-version update remains fail-closed because the Application owner exposes configuration/restart operations but no canonical artifact-version migration. Status and uninstall likewise resolve the canonical Application instance and delegate to the Application owner.

**Legacy manifestless** Application catalog entries remain `manual` for compatibility. They are discoverable and previewable, but no automatic Marketplace install/update/uninstall route is synthesized for them. Marketplace persists only source/version/provenance/dependency distribution evidence for manifest-backed installs; installed/running definitions and instances remain exclusively owned by `ApplicationRepository` and the `applications` Control Plane resources.
