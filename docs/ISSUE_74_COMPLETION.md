...
| Canonical browser session/context | `BrowserSessionRef` / `BrowserSessionScope`, with owner/project/task/run/agent scope fields, bounded expiry, privacy classification, allowed domains and namespaced adapter metadata |
...
| Upload/download file bridge | uploads consume authorized canonical `file_*` refs through `FileProvider`; downloads create canonical `file_*` refs, link a canonical `artifact_*` through the refined #13 file/artifact seam, and record SHA-256, content type and redacted provenance |
| Authorization/network policy | capability safety/side-effect classifications, canonical policy/approval pipeline, `BrowserNetworkPolicyHook`, SSRF/private-network and redirect checks; synchronous DNS/policy resolution runs off the async event loop |
...
| Observability/evidence | `browser.operation` adapter metadata plus canonical `InvocationRecord` propagation, including timeout/cancellation; downloads use `result_ref`, `artifact_refs` and `evidence_refs` |
...
- [x] **File downloads become canonical File/Artifact resources with provenance.** The browser
  ...
  the artifact to the file through the refined #13 `link_artifact` seam, records a redacted source
  URL, content type, SHA-256, timestamp, provenance and trust classification, and returns the file
  and artifact through canonical result/evidence fields. No browser-provider-private artifact ID
  or parallel artifact store is introduced.
...
- [x] **Network/domain policy hooks exist.** The default policy supports scheme restrictions,
  ...
  blocking, response-size limits and redirect re-checks. Blocking DNS resolution occurs inside
  the browser worker thread rather than on the shared asyncio event loop.
...
| file download/artifact creation | `test_download_enters_canonical_file_and_artifact_path_with_redacted_provenance` verifies `file_*`, `artifact_*`, File↔Artifact linking, result/artifact/evidence refs and secret-free persisted provenance |
...
| timeout/cancellation | `test_timeout_and_cancellation_use_canonical_errors_and_browser_metadata` verifies canonical errors plus redacted browser-operation metadata in the error and invocation record |
| blocked domain/network policy | `test_network_policy_blocks_private_target`; `test_network_policy_checks_run_off_event_loop` verifies synchronous policy/DNS work is not run on the event-loop thread |
...
| abandoned session cleanup | `test_session_ttl_evicts_abandoned_sessions` |
...
| URL/evidence redaction and observability | `test_browser_operation_metadata_is_redacted_and_enters_invocation_record` plus the download and timeout/cancellation tests |

## Security and runtime invariants

- Raw cookies and credentials are not canonical payloads.
- Browser sessions are owner/project isolated and receive a bounded default TTL; expired sessions are evicted on access/session creation.
...
- Blocking hostname resolution and network-policy evaluation run in the reference browser worker thread, not on the shared asyncio event loop.
- Browser operation telemetry strips URL credentials, query strings and fragments before storing ordinary invocation metadata.
- Persisted browser-download provenance likewise strips credentials, query strings and fragments from source URLs.
- Provider errors retain canonical `ErrorCode` semantics; browser metadata is additive and namespaced.
- Invoker-owned timeout/cancellation records can collect provider metadata through the optional generic `InvocationFailureMetadataProvider` seam; the core remains browser-neutral.
...
- The reference adapter verifies artifact-link support before network/download persistence so an incompatible minimal file provider cannot leave an orphan file merely because the artifact path is unavailable.
- Downloaded bytes cross the canonical `FileProvider` seam and the resulting file is linked to a canonical `artifact_*` through the existing #13 file/artifact lifecycle rather than a browser-private artifact path.
...
`main` base and actionable review findings are resolved. The remaining follow-up issues listed in
#74 (#14, #16, #34, #37, #43, #17/#73) may consume or harden this boundary later; they are not
prerequisites for the browser capability contract itself.
