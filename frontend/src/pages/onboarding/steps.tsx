  ...
  return (
    <>
      ...
      {status.state === "needs_selection" || status.state === "ready_for_task" ? (
        <Card title="Optional legacy single-Agent first task">
          <p>The multi-agent workflow above is the supported product first run. This path remains available for compatibility and focused General Assistant testing.</p>
...
function GeneralAssistantStep(props: OnboardingStepsProps) {
  ...
  return (
    <Card title="Optional General Assistant setup">
      <p>The official first run no longer requires a General Assistant. Install one only if you also want the editable single-Agent compatibility path.</p>
