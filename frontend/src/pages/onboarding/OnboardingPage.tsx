...
export function OnboardingPage({ client, onboarding, session, manifest }: OnboardingPageProps) {
  ...
  const runMultiAgent = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const projectId = optionalText(form, "project_id");
      ...
      setNotice("The official multi-agent first run completed. Plan, specialized roles, results, artifacts, review, and trace are shown below.");
      await load();
    } catch (error) {
      setActionError(error);
    } finally {
  ...
  const runFirstTask = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setNotice("The legacy single-Agent Task completed and produced a Result.");
      await load();
    } catch (error) {
      setActionError(error);
    } finally {
  ...
  return (
    <div className="stack">
      <header className="page-header">
        ...
        <p>The supported first-run experience is a real multi-agent goal executed through the same canonical Control Plane resources and commands as the CLI. It never contacts a model backend directly and never selects a remote or paid provider implicitly.</p>
      </header>
