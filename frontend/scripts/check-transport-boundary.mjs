...
for (const file of readdirSync(apiDirectory)) {
  ...
  const invokesDirectFetch = hasDirectGlobalFetchCall(content);
...
function hasDirectGlobalFetchCall(content) {
  // Match actual call contexts without confusing a domain method declaration such
  // as `RepositoryCollectionClient.fetch(...)` for the browser global `fetch(...)`.
  return (
    /\bglobalThis\.fetch\s*\(/.test(content)
    || /\b(?:await|return|void)\s+fetch\s*\(/.test(content)
    || /=\s*fetch\s*\(/.test(content)
    || /(?:\(|,)\s*fetch\s*\(/.test(content)
  );
}
