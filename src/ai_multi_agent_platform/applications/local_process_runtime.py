...
class LocalApplicationHostProfile:
    """Backend-private suitability facts for the current local execution host."""
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "operating_system", _canonical_operating_system(self.operating_system))
...
def detect_local_application_host_profile(
    *,
    ...
    """Detect conservative local host facts with stdlib-only probes.

    GPU inventory, custom labels and specialized capabilities are intentionally not
    guessed. A deployment with authoritative hardware inventory may inject an explicit
    profile instead.
    """
...
class LocalProcessApplicationRuntime:
    ...
    def _validate_host_requirements(self, request: ApplicationInstallRequest) -> None:
        requirements = request.manifest.resources
        ...
        available_capabilities = set(host.capabilities) | set(self.descriptor.capabilities)
