    ...
    def safe_value(self) -> Any:
        if self.is_secret:
            if isinstance(self.value, SecretReference):
                return {"configured": True, "secret_ref": _safe_secret_reference(self.value)}
...
def _safe_serialize(value: Any) -> Any:
    if isinstance(value, SecretReference):
        return {"secret_ref": _safe_secret_reference(value)}
...
def _safe_secret_reference(reference: SecretReference) -> dict[str, Any]:
    redacted = redact_sensitive(reference)
    if not isinstance(redacted, dict):
        raise ConfigurationError("secret reference redaction returned an invalid representation")
