"""Versioned Control Plane adapters for issue #872 coding-batch composition state."""
...
from .models import BatchAggregationPolicy, CodingBatch, CodingWorkItem
...
class CodingBatchCommandHandlers:
    """Mutation helpers behind the generic authenticated/authorized Control Plane command seam."""
...
def _parse_work_item(raw_value: JsonValue) -> CodingWorkItem:
    ...
    return CodingWorkItem(
        work_item_id=_required_mapping_string(raw, "work_item_id"),
        task_id=_required_mapping_string(raw, "task_id"),
        plan_id=_required_mapping_string(raw, "plan_id"),
        step_id=_required_mapping_string(raw, "step_id"),
        dependencies=_mapping_string_tuple(raw, "dependencies"),
        affected_paths=_mapping_string_tuple(raw, "affected_paths"),
        semantic_scopes=_mapping_string_tuple(raw, "semantic_scopes"),
...
def _required_mapping_string(payload: Mapping[str, JsonValue], key: str) -> str:
...
def _mapping_string_tuple(payload: Mapping[str, JsonValue], key: str) -> tuple[str, ...]:
    value = payload.get(key)
    if value is None:
        return ()
...
def _optional_string_tuple(
    ...
    if payload.get(key) is None:
        return None
    return _mapping_string_tuple(payload, key)
