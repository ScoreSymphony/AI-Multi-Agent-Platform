...
class LearningQualityGate:
    ...
    def _enforce_evaluation(self, candidate: LearningCandidate) -> None:
        ...
        for run_id in candidate.evaluation_run_ids:
            ...
            if (
                candidate.gate_plan.require_regression_free
                and detail.comparison is not None
                and detail.comparison.regressions
            ):
                raise ContractError(
                    ErrorCode.CONFLICT,
                    ...
                    details={
                        "evaluation_run_id": run_id,
                        "regression_count": len(detail.comparison.regressions),
                    },
                )
