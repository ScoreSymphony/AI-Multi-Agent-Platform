...
def _authentication_route_error(method: str, relative: str) -> APIException | None:
    """Classify auth URI existence/method support from the documented auth surface."""

    normalized_path = f"/api/{API_VERSION}{relative}".rstrip("/") or "/"
    allowed_methods: set[str] = set()
    for template, methods in _authentication_route_table():
        if _openapi_path_matches(template, normalized_path):
            allowed_methods.update(methods)

    if not allowed_methods:
        return APIException(status=404, code="not_found", message="route not found")
...
@lru_cache(maxsize=1)
def _authentication_route_table() -> tuple[tuple[str, frozenset[str]], ...]:
    """Freeze the route table generated by the canonical authentication OpenAPI owner."""

    specification = _augment_authentication_openapi({"paths": {}}, cookie_name="amp_session")
    ...
    for template, operations in paths.items():
        if not isinstance(template, str) or not isinstance(operations, dict):
            continue
        methods = frozenset(
            operation.upper()
            for operation in operations
            if isinstance(operation, str) and operation.lower() in http_methods
        )
        if methods:
            routes.append((template, methods))
    return tuple(routes)
