...
class AuthenticatedControlPlaneHTTP(_ControlPlaneHTTP):
    ...
    async def handle(self, request: HTTPRequest) -> HTTPResponse:
        request_id = _header(request.headers, "x-request-id") or "request-authentication"
        ...
        try:
            ...
            if relative.startswith("/auth/"):
                return self._authenticated_auth_route(
                    request,
                    relative,
                    actor,
...
def _with_authenticated_actor(request: HTTPRequest, actor: AuthenticatedActor) -> HTTPRequest:
    headers = {
        key: value
