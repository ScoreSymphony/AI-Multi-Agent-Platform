...
class SkillMarketplaceKindHandler:
    ...
    @staticmethod
    def _provenance(item: RegistryItem) -> Provenance:
        return Provenance(
            source="marketplace",
            details={
    ...
    @staticmethod
    def _belongs_to_item(revision: SkillRevision, item: RegistryItem) -> bool:
...
class ApplicationMarketplaceKindHandler:
    ...
    def _source_ref(item: RegistryItem) -> str:
        return f"marketplace:{item.item_id}@{item.version}"
