...
def _budget_runtime() -> Any:
    """Load execution-budget composition types without a static package-cycle edge."""

    return import_module("ai_multi_agent_platform.execution.budgets")
...
def extend_single_node_with_task_budgets(
    ...
    *,
    ...
    budget = _budget_runtime()
    task_budgets = budget.TaskBudgetEnforcementService(
        budget.SQLiteTaskBudgetStore(config.database_dir / "task-execution-budgets.sqlite3"),
        accounting_service,
    )
    task_budget_mutations = budget.TaskBudgetPolicyMutationService(
        task_budgets,
        base.approval_gate,
    )
    budget.register_task_budget_control_plane(
        base.control_plane,
        task_budgets,
    ...
    budgeted_models = budget.TaskBudgetModelRuntime(base.model_runtime, task_budgets)
    base.model_runtime = budget.as_model_runtime(budgeted_models)
    ...
    if capability_turn is not None:
        ...
        capability_turn._invoker = budget.as_capability_invoker(  # noqa: SLF001 - composition seam
            budget.TaskBudgetCapabilityInvoker(  # noqa: SLF001
                capability_turn._invoker, task_budgets
            )
        )
