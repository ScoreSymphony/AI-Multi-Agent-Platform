...
def install_single_node_context(
    ...
    *,
    ...
    """Install the canonical Context Bundle path through public composition seams.

    The installer replaces only the deployment lifecycle binding. Every canonical owner remains
    unchanged and one #15 authorization wrapper stays the execution enforcement boundary after
    canonical Task Project scope is resolved. The kernel itself is never introspected or mutated.
    ...
    lifecycle = CanonicalContextAgentLifecycleBackend(
        delegate=base.execution_fallback_lifecycle,
        tasks=tasks,
