...
class OnboardingResourceService:
    ...
    async def _status(self, context: RequestContext) -> dict[str, JsonValue]:
        status = dict(await self.onboarding.status_async(context))
...
class SetupSessionResourceService:
    ...
    async def list_resources(
        ...
        del query
        return (await self.setup.status_async(context),)
