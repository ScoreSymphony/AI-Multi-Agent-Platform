...
def collect_model_inventory(models: ModelRegistry) -> FirstRunModelInventory:
    ...
    local_models = tuple(model for model in enabled_models if model.location is ModelLocation.LOCAL)
...
def _model_guidance(inventory: FirstRunModelInventory) -> list[JsonValue]:
    ...
    if not inventory.golden_path_models:
        ...
        guidance.append(
            ...
            "refresh-health PROVIDER_ID`) or revalidate the endpoint before starting the first "
            "Task."
        )
    guidance.append(
