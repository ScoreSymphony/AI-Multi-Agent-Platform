...
from .models import BuildSpecification, BuildTarget, PackageType, ReleaseChannel, ReleaseVisibility
...
class ApplicationReleaseResourceService:
    ...
    async def list_resources(
        ...
        del context, query
        return tuple(json_object(release) | {"id": release.release_id, "type": "application_release"}
    ...
    async def get_resource(
        ...
        return json_object(release) | {"id": release.release_id, "type": "application_release"}
...
class ApplicationReleaseCommandHandlers:
    ...
    async def create(
        ...
        if resource_ref != APPLICATION_RELEASE_COLLECTION:
            raise ContractError(ErrorCode.INVALID_REQUEST, "create requires application-releases resource_ref")
    ...
    async def preview(
        ...
        return {"id": resource_ref, "type": "application_release_preview", **preview}
...
def _resource(release: object) -> dict[str, JsonValue]:
    ...
    if not isinstance(release_id, str):
        raise ContractError(ErrorCode.CONTRACT_VIOLATION, "release projection has no release_id")
...
def _actor(context: RequestContext) -> ActorIdentity:
    ...
    try:
        return ActorIdentity(context.actor.principal_ref, ActorType(context.actor.actor_type))
    except ValueError as exc:
        raise ContractError(ErrorCode.INVALID_REQUEST, "unsupported authenticated actor type") from exc


def _build_spec(value: JsonValue | None) -> BuildSpecification:
    if not isinstance(value, dict):
        raise ContractError(ErrorCode.INVALID_REQUEST, "build_specification must be an object")
    command = _string_tuple(value.get("command"), "command")
    raw_targets = value.get("targets")
    if not isinstance(raw_targets, list):
        raise ContractError(ErrorCode.INVALID_REQUEST, "build_specification.targets must be an array")
    targets: list[BuildTarget] = []
    for raw in raw_targets:
        if not isinstance(raw, dict):
            raise ContractError(ErrorCode.INVALID_REQUEST, "each build target must be an object")
        try:
            targets.append(
                BuildTarget(
                    ...
                    required_capabilities=_string_tuple(raw.get("required_capabilities"), "required_capabilities"),
                )
            )
        except ValueError as exc:
            raise ContractError(ErrorCode.INVALID_REQUEST, f"invalid build target: {exc}") from exc
    try:
        return BuildSpecification(
            ...
            pre_build_checks=_string_tuple(value.get("pre_build_checks"), "pre_build_checks"),
            test_gates=_string_tuple(value.get("test_gates"), "test_gates"),
            post_build_checks=_string_tuple(value.get("post_build_checks"), "post_build_checks"),
            required_capabilities=_string_tuple(value.get("required_capabilities"), "required_capabilities"),
            resource_hints=_json_object(value.get("resource_hints"), "resource_hints"),
            secret_references=_string_tuple(value.get("secret_references"), "secret_references"),
        )
    except ValueError as exc:
        raise ContractError(ErrorCode.INVALID_REQUEST, f"invalid build specification: {exc}") from exc
...
def _string_tuple(value: JsonValue | None, field: str) -> tuple[str, ...]:
    ...
    if not isinstance(value, list) or any(not isinstance(item, str) for item in value):
        raise ContractError(ErrorCode.INVALID_REQUEST, f"{field} must be an array of strings")
    return tuple(value)
