...
class TaskBudgetEnforcementService(TaskBudgetAdmission):
    ...
    async def admit(
        ...
        *,
        ...
        policy = await asyncio.to_thread(self._store.get_policy, task_id)
        if policy is None:
            return _no_policy_decision(task_id, action)
        ...
        blocked = _pre_reservation_decision(task_id, action, snapshot, relevant)
        if blocked is not None:
            return blocked

        claims, reservations, rejected = _reservation_claims(
            policy=policy,
            snapshot=snapshot,
            task_id=task_id,
            action=action,
        ...
        raced = await self._reserve_claims(task_id, action, requested, claims)
        if raced is not None:
            return raced

        admitted = await self.snapshot(task_id)
        return _admitted_decision(task_id, action, relevant, admitted, reservations)
    ...
    async def _reserve_claims(
        ...
        if not claims:
            return None
        accepted = await asyncio.to_thread(self._store.try_reserve_many, claims)
        if accepted:
            return None
    ...
    async def assess_after_action(
        ...
        policy = await asyncio.to_thread(self._store.get_policy, decision.task_id)
        if policy is None:
            return _no_policy_decision(decision.task_id, decision.action)
    ...
    for dimension, quantity in requested.items():
        ...
        claims.append(
            ReservationClaim(
                ...
                limit=limit.limit,
                external_consumed=(
                    state.consumed if limit.source is BudgetConsumptionSource.ACCOUNTING else 0.0
                ),
                include_runtime_counter=(limit.source is BudgetConsumptionSource.RUNTIME_COUNTER),
            )
        )
    return tuple(claims), tuple(reservations), None
...
def _admitted_decision(
    ...
    warning = next(
        (item for item in snapshot.dimensions if item.limit.dimension in relevant and item.warning),
        None,
    )
    return BudgetAdmissionDecision(
        task_id=task_id,
