...
class AgentCapabilityTurn:
    ...
    async def execute(
        ...
        *,
        ...
        budget_models = import_module("ai_multi_agent_platform.execution.budgets.models")
        ...
        model_budget = await self._admit_budget(
            task_id=task_id,
            action=budget_models.BudgetActionKind.MODEL_CALL,
            quantities={budget_models.BudgetDimension.MODEL_CALLS: 1.0},
            run_id=run_id,
            agent_id=agent_id,
        ...
        for ordinal, call in enumerate(response.tool_calls, start=1):
            ...
            tool_budget = await self._admit_budget(
                task_id=task_id,
                action=budget_models.BudgetActionKind.TOOL_CALL,
                quantities={budget_models.BudgetDimension.TOOL_CALLS: 1.0},
                run_id=run_id,
                agent_id=agent_id,
                agent_run_id=agent_run_id,
    ...
    async def _reconcile_budget(self, decision: Any | None) -> None:
