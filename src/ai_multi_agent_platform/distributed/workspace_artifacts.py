...
_WORKER_WORKSPACE_ARTIFACT_NAMESPACE = "ai-multi-agent-platform:worker-workspace-artifact"
...
class ArtifactPublishingWorkerDispatcher:
    ...
    def evidence(self, worker_job_id: str) -> WorkspaceDispatchEvidence:
        ...
        enriched = replace(
            evidence.result,
            artifact_ids=tuple(dict.fromkeys((*evidence.result.artifact_ids, *artifact_ids))),
        )
...
def _artifact_id(
    ...
    return f"artifact_{uuid5(NAMESPACE_URL, f'{_WORKER_WORKSPACE_ARTIFACT_NAMESPACE}:{identity}')}"
