...
class CoordinationEvaluationEvidenceProvider:
    ...
    def __init__(self, repository: CoordinatorRepository) -> None:
        ...
        self._runtime_repository = runtime_coordinator_repository(repository)
    ...
    async def async_collect(self, *, task_id: str, run_id: str) -> EvaluationEvidence:
        matches = tuple(
            state
            for state in await self._runtime_repository.list_active_plans()
            if state.plan.task_id == task_id and state.plan.active
        )
        if not matches:
            return EvaluationEvidence()
        ...
        records = {
            ...
            for record in await self._runtime_repository.list_step_records(state.plan.id)
        }
        return _coordination_evidence(
            task_id=task_id,
            run_id=run_id,
