...
class SqliteDeliveryAttemptRepository(DeliveryAttemptRepository):
    """Durable delivery history preserving retry/dedupe identity across restarts."""
    ...
    def __init__(self, db_path: str | Path) -> None:
        ...
        try:
            ...
            raise ContractError(
                ...
                "failed to initialize notification delivery storage",
            ) from exc
    ...
    async def save(self, attempt: DeliveryAttempt) -> DeliveryAttempt:
        ...
        try:
            ...
            raise ContractError(
                ErrorCode.BACKEND_ERROR,
                "failed to persist notification delivery attempt",
            ) from exc
        return attempt
    ...
    async def latest(self, notification_id: str, channel: str) -> DeliveryAttempt | None:
        ...
        try:
            ...
            raise ContractError(
                ErrorCode.BACKEND_ERROR,
                "failed to read notification delivery attempt",
            ) from exc
    ...
    async def list_for_notification(self, notification_id: str) -> tuple[DeliveryAttempt, ...]:
        ...
        try:
            ...
            raise ContractError(
                ErrorCode.BACKEND_ERROR,
                "failed to list notification delivery attempts",
            ) from exc
...
def _decode(payload: str) -> DeliveryAttempt:
    ...
    if attempted_at.utcoffset() is None:
        raise ValueError("delivery-attempt timestamp must be timezone-aware")
...
__all__ = ["SqliteDeliveryAttemptRepository"]
