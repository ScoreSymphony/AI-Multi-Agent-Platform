"""Dedicated awaitable persistence boundary for Governance runtime paths."""
...
class GovernancePersistenceOffload:
    """Serialize and bound one synchronous Governance persistence backing store."""

    def __init__(
        self,
        *,
        thread_name_prefix: str = "governance-persistence",
...
_GOVERNANCE_REPOSITORY_OFFLOADS = SharedPersistenceOffloadRegistry[GovernancePersistenceOffload]()


def governance_persistence_offload(
    repository: object,
    *,
    owner: object,
    requested: GovernancePersistenceOffload | None = None,
) -> GovernancePersistenceOffload:
    """Resolve the shared serialization boundary for one Governance repository identity."""

    return _GOVERNANCE_REPOSITORY_OFFLOADS.resolve(
        repository,
        owner=owner,
        requested=requested,
        factory=GovernancePersistenceOffload,
    )
...
__all__ = ["GovernancePersistenceOffload", "governance_persistence_offload"]
