...
This script prepares and validates evidence for the real two-VPS private-tunnel
acceptance path. It deliberately does not provision infrastructure, mutate canonical
platform state, or replace the #14/#35/#36 runtime contracts. Addresses and credential
material are consumed only at runtime and are never written to evidence reports.
"""
...
_PHASES = {"registration", "dispatch", "interruption", "recovery", "restart", "security"}
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    recovery.add_argument("--post-recovery-status", choices=("succeeded", "failed"), required=True)
    ...
    restart.add_argument("--post-restart-status", choices=("succeeded", "failed"), required=True)
    ...
    security.add_argument("--unauthenticated-registration-rejected", type=_bool_arg, required=True)
...
def _base_phase(args: argparse.Namespace, phase: str) -> dict[str, object]:
...
def _record_registration(args: argparse.Namespace) -> dict[str, object]:
    payload = _base_phase(args, "registration")
    passed = bool(args.authenticated and args.heartbeat_healthy and args.cpu_cores > 0 and args.ram_bytes > 0)
    payload.update(
        {
            "status": "pass" if passed else "fail",
            "node_id": str(args.node_id),
            "worker_id": str(args.worker_id),
            "authentication_mode": str(args.authentication_mode),
            "authenticated": bool(args.authenticated),
            "heartbeat_healthy": bool(args.heartbeat_healthy),
            "capability_refs": sorted(set(str(value) for value in args.capability)),
            "cpu_cores": int(args.cpu_cores),
            "ram_bytes": int(args.ram_bytes),
        }
    )
    return payload


def _record_dispatch(args: argparse.Namespace) -> dict[str, object]:
    payload = _base_phase(args, "dispatch")
    correlation_preserved = args.correlation_id == args.returned_correlation_id
    trace_preserved = args.trace_id == args.returned_trace_id
    passed = (
        args.result_status == "succeeded"
        and args.duplicate_run_count == 0
        and args.selected_by_capability_policy
        and correlation_preserved
        and trace_preserved
    )
    payload.update(
        {
            ...
            "worker_job_id": str(args.worker_job_id),
            "result_status": str(args.result_status),
            "correlation_id": str(args.correlation_id),
            "returned_correlation_id": str(args.returned_correlation_id),
            "trace_id": str(args.trace_id),
            "returned_trace_id": str(args.returned_trace_id),
            "correlation_preserved": correlation_preserved,
            "trace_preserved": trace_preserved,
            "duplicate_run_count": int(args.duplicate_run_count),
            "selected_by_capability_policy": bool(args.selected_by_capability_policy),
        }
    )
    return payload


def _record_interruption(args: argparse.Namespace) -> dict[str, object]:
    payload = _base_phase(args, "interruption")
    passed = all(
        (
            args.heartbeat_expired,
            args.worker_unavailable,
            args.new_placement_blocked,
            args.active_state_reconciled,
            not args.indefinite_running_state,
        )
    )
    payload.update(
        {
            "status": "pass" if passed else "fail",
            "worker_id": str(args.worker_id),
            "heartbeat_expired": bool(args.heartbeat_expired),
            "worker_unavailable": bool(args.worker_unavailable),
            "new_placement_blocked": bool(args.new_placement_blocked),
            "active_state_reconciled": bool(args.active_state_reconciled),
            "indefinite_running_state": bool(args.indefinite_running_state),
        }
    )
    return payload


def _record_recovery(args: argparse.Namespace) -> dict[str, object]:
    payload = _base_phase(args, "recovery")
    passed = all(
        (
            args.re_registered,
            args.heartbeat_recovered,
            args.stale_authority_rejected,
            args.duplicate_run_count == 0,
            args.post_recovery_status == "succeeded",
        )
    )
    payload.update(
        {
            "status": "pass" if passed else "fail",
            "worker_id": str(args.worker_id),
            "re_registered": bool(args.re_registered),
            "heartbeat_recovered": bool(args.heartbeat_recovered),
            "stale_authority_rejected": bool(args.stale_authority_rejected),
            "duplicate_run_count": int(args.duplicate_run_count),
            "post_recovery_run_id": str(args.post_recovery_run_id),
            "post_recovery_worker_job_id": str(args.post_recovery_worker_job_id),
            "post_recovery_status": str(args.post_recovery_status),
        }
    )
    return payload


def _record_restart(args: argparse.Namespace) -> dict[str, object]:
    payload = _base_phase(args, "restart")
    process_restarted = args.first_process_evidence_ref != args.second_process_evidence_ref
    passed = all(
        (
            process_restarted,
            args.re_registered,
            args.heartbeat_recovered,
            args.post_restart_status == "succeeded",
        )
    )
    payload.update(
        {
            "status": "pass" if passed else "fail",
            "worker_id": str(args.worker_id),
            "first_process_evidence_ref": str(args.first_process_evidence_ref),
            "second_process_evidence_ref": str(args.second_process_evidence_ref),
            "worker_process_restarted": process_restarted,
            "re_registered": bool(args.re_registered),
            "heartbeat_recovered": bool(args.heartbeat_recovered),
            "post_restart_run_id": str(args.post_restart_run_id),
            "post_restart_status": str(args.post_restart_status),
        }
    )
    return payload


def _record_security(args: argparse.Namespace) -> dict[str, object]:
    payload = _base_phase(args, "security")
    passed = all(
        (
            args.worker_ports_publicly_closed,
            args.unauthenticated_registration_rejected,
            args.invalid_credential_rejected,
            args.authorization_server_side,
            not args.reusable_secrets_found,
            not args.provider_metadata_canonicalized,
        )
    )
    payload.update(
        {
            "status": "pass" if passed else "fail",
            "worker_ports_publicly_closed": bool(args.worker_ports_publicly_closed),
            "unauthenticated_registration_rejected": bool(args.unauthenticated_registration_rejected),
            "invalid_credential_rejected": bool(args.invalid_credential_rejected),
            "authorization_server_side": bool(args.authorization_server_side),
            "reusable_secrets_found": bool(args.reusable_secrets_found),
            "provider_metadata_canonicalized": bool(args.provider_metadata_canonicalized),
        }
    )
    return payload
...
def _record_phase(args: argparse.Namespace) -> dict[str, object]:
    handlers = {
        "record-registration": _record_registration,
        "record-dispatch": _record_dispatch,
        "record-interruption": _record_interruption,
        "record-recovery": _record_recovery,
        "record-restart": _record_restart,
        "record-security": _record_security,
    }
    payload = handlers[args.command](args)
    _write_json(args.json_report, payload)
    if payload["status"] != "pass":
        raise AcceptanceError(f"{payload['phase']} evidence records a failing acceptance phase")
    return payload
...
def _validate_phase(payload: Mapping[str, object], phase: str) -> None:
    ...
    if payload.get("phase") != phase:
        raise AcceptanceError(f"expected {phase} phase evidence")
...
def _validate_probes(
    reports: Sequence[Mapping[str, object]], *, platform_commit: str, worker_host_label: str
    ...
    for label in _ENDPOINT_LABELS:
        ...
        if private_report.get("expected") != "reachable" or private_report.get("reachable") is not True:
...
def _validate_transport_report(
    report: Mapping[str, object] | None, *, expected_worker_id: str
    ...
    if not isinstance(transport.get("authentication"), str):
...
def _finalize(args: argparse.Namespace) -> dict[str, object]:
    ...
    if not _required_bool(dispatch, "correlation_preserved") or not _required_bool(
        dispatch, "trace_preserved"
    ):
        raise AcceptanceError("correlation/trace context did not survive remote dispatch")
    ...
    if not _required_bool(restart, "re_registered") or not _required_bool(
        restart, "heartbeat_recovered"
    ):
        raise AcceptanceError("Worker restart did not recover registration/heartbeat")
    if restart.get("post_restart_status") != "succeeded":
        raise AcceptanceError("post-restart deterministic dispatch did not succeed")

    security_required_true = (
        "worker_ports_publicly_closed",
        "unauthenticated_registration_rejected",
        "invalid_credential_rejected",
        "authorization_server_side",
    )
    if not all(_required_bool(security, key) for key in security_required_true):
    ...
    report: dict[str, object] = {
        ...
        "canonical": {
            ...
            "post_recovery_worker_job_id": _required_string(
                recovery, "post_recovery_worker_job_id"
            ),
