...
- Copilot CLI — `github/copilot-cli`, non-archived. The custom GitHub Copilot CLI License permits install/run and limited redistribution of unmodified copies but explicitly withholds modification and derivative-work rights. The official README requires an active Copilot subscription and authenticated GitHub access, so the card is `deferred`, `evaluation:not-required`, `cost:incompatible` under the project's recurring-cost policy, and `network:required`.
- Claude Code — `anthropics/claude-code`, non-archived. The repository license states all rights reserved and subjects use to Anthropic's Commercial Terms of Service. The card is `deferred`, `evaluation:not-required`, `cost:incompatible`, and explicitly records the Anthropic provider/commercial-terms dependency rather than presenting Claude Code as an open-source candidate.
