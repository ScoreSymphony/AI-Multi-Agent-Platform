...
def test_git_discovery_is_provider_neutral_and_forces_unknown_review_for_changed_head() -> None:
    ...
    revisions = {
        entry.source_url: _remote_revision(entry.revision) for entry in inventory.entries
    }
    ...
    candidate = next(
        item for item in result.observations if item.component == current.component
    )
...
def test_git_discovery_recognizes_composite_pin_containing_same_commit() -> None:
    ...
    observed = next(
        item for item in result.observations if item.component == composite.component
    )
