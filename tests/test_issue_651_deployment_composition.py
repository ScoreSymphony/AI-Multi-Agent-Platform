...
def test_public_single_node_deployment_owns_durable_handoff_runtime(tmp_path: Path) -> None:
    ...
    assert not any(command.startswith("handoff") for command in deployment.control_plane.registered_commands)
...
def test_public_single_node_restart_reopens_same_handoff_state_paths(tmp_path: Path) -> None:
    ...
    assert first.handoffs.context_bundle_repository.path == second.handoffs.context_bundle_repository.path
    assert first.handoffs.context_binding_repository.path == second.handoffs.context_binding_repository.path
