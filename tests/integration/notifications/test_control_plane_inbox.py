...
from ai_multi_agent_platform.testing import FakeAuthorizationProvider, FakeLifecycleBackend, FakeOrchestrator
...
def _context(user_id: str, *, key: str | None = None) -> RequestContext:
    return RequestContext(
        ...
        actor=ActorContext(principal_ref=user_id, owner_type="user", owner_id=user_id),
        idempotency_key=key,
    )
...
def test_control_plane_inbox_is_recipient_scoped_and_commands_are_idempotent() -> None:
    async def scenario() -> None:
        ...
        page = await control_plane.list_extension_resources(_context(user_id), "notifications", PageQuery())
        assert page["total"] == 1
        item = page["items"][0]
        assert item["id"] == created.id
        assert item["summary"]["token"] == "[REDACTED]"
        hidden = await control_plane.list_extension_resources(_context(other_user_id), "notifications", PageQuery())
        assert hidden["total"] == 0
        read = await control_plane.execute_command(_context(user_id, key="read-once"), "notification.mark-read", created.id)
        reread = await control_plane.execute_command(_context(user_id, key="read-again"), "notification.mark-read", created.id)
        assert read["state"] == "read"
        assert reread["state"] == "read"
        assert reread["read_at"] == read["read_at"]
        preferences = await control_plane.list_extension_resources(_context(user_id), "notification-preferences", PageQuery())
...
def test_control_plane_preference_update_cannot_target_another_recipient() -> None:
    async def scenario() -> None:
        ...
        user_id = new_id("user")
        other_user_id = new_id("user")
        updated = await control_plane.execute_command(_context(user_id, key="preference-update"), "notification.preference.update", user_id, {"minimum_severity": "warning", "enabled_categories": ["task", "approval"], "muted": True})
        assert updated["minimum_severity"] == "warning"
        assert updated["muted"] is True
        try:
            await control_plane.execute_command(_context(user_id, key="preference-cross-user"), "notification.preference.update", other_user_id, {"muted": True})
...
def test_authenticated_http_ignores_spoofed_owner_headers_for_notification_inbox() -> None:
    async def scenario() -> None:
        repository = InMemoryKernelRepository()
        control_plane = ControlPlane(kernel=PlatformKernel(orchestrator=FakeOrchestrator(), lifecycle=FakeLifecycleBackend(), repository=repository), events=repository)
        auth = LocalAuthenticationService(password_hasher=ScryptPasswordHasher(n=2**10, r=8, p=1, maxmem=8 * 1024 * 1024))
        ...
        response = await http.handle(HTTPRequest(method="GET", path="/api/v1/notifications", headers={"authorization": f"Bearer {token.secret}", "x-principal-ref": other_user_id, "x-owner-type": "user", "x-owner-id": other_user_id}))
