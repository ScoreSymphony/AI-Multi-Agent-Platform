...
def _service(
    ...
    registry = DistributionSetupRegistryPort(
        cast(DistributionService, distribution),
        cast(RegistryCommandHandlers, commands),
    )
    ...
    asyncio.run(
        lifecycle.update_session(
            ...
            "initial-setup",
            {
                ...
                "registry_items": [{"item_id": item_id, "version": "1.0"} for item_id in item_ids],
            },
        )
    )


def test_partial_failure_persists_across_restart_replays_final_failure_and_retries(
    tmp_path,
) -> None:
    ...
    failed = asyncio.run(lifecycle.provision(_context("operation-1"), "initial-setup", {}))
    ...
    replayed = asyncio.run(restarted.provision(_context("operation-1"), "initial-setup", {}))
    assert replayed["replayed"] is True
    assert replayed["outcome"]["action_id"] == "registry:beta@1.0"
    assert replayed["outcome"]["state"] == "failed"
    assert commands.activations == ["alpha@1.0", "beta@1.0"]

    retried = asyncio.run(restarted.provision(_context("operation-2"), "initial-setup", {}))
...
def test_restart_reconciles_owner_installation_even_if_setup_outcome_was_not_saved(
    tmp_path,
) -> None:
