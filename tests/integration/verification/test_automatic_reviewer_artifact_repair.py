...
_ARTIFACT_CAPABILITY_ID = "tool.test.write_repair_artifact"
_ARTIFACT_TOOL_REF = "test.write_repair_artifact"


class _ArtifactRepairTransport:
    """Drive Artifact v1 -> NEEDS_CHANGES -> Artifact v2 -> PASS locally."""
        ...
        *,
        ...
        if self.chat_calls in {1, 3}:
            ...
            if self.chat_calls == 1:
                ...
                content = "artifact-v2: repaired conclusion present\n"
                call_id = "call-artifact-v2"
        ...
        if self.chat_calls == 2:
            ...
            review = {
                ...
                "findings": [
                    {
                        "code": "artifact_repair_verified",
                        "message": "The repaired Artifact satisfies the requirement.",
                        "severity": "info",
                    }
                ],
            }
...
class _CanonicalArtifactProvider(CapabilityToolProvider):
    """Test provider using the real canonical FileProvider boundary."""
    ...
    def descriptor(self) -> ProviderDescriptor:
        return ProviderDescriptor(
            provider_id="test.canonical-artifact",
        ...
        return (
            CapabilityRegistration(
                capability=CapabilitySpec(
                    ...
                    name="Write canonical repair Artifact",
    ...
    async def invoke(self, invocation: ToolInvocation) -> ToolResult:
        ...
        owner_id = invocation.context.owner_id or "automatic-reviewer-artifact-repair"
...
def _model_payload() -> dict[str, JsonValue]:
    return {
        ...
        "display_name": "Local Artifact Repair Model",
...
def _producer_profile() -> AgentProfile:
    return AgentProfile(
        name="Canonical Artifact Repair Producer",
...
def test_productive_artifact_repair_creates_fresh_file_backed_subject(tmp_path: Path) -> None:
    async def scenario() -> None:
        transport = _ArtifactRepairTransport()
        ...
        token = deployment.authentication.create_personal_access_token(
            admin.user_id,
            purpose="automatic-artifact-repair-integration",
        ).secret
        ...
        configured = await deployment.http.handle(
            HTTPRequest(
                ...
                headers=_headers(token, key="automatic-artifact-repair:model"),
                body=_model_payload(),
            )
        )
        ...
        project_response = await deployment.http.handle(
            HTTPRequest(
                ...
                headers=_headers(token, key="automatic-artifact-repair:project"),
                body={"name": "Automatic Artifact repair project"},
            )
        )
        ...
        workspace_response = await deployment.http.handle(
            HTTPRequest(
                ...
                headers=_headers(token, key="automatic-artifact-repair:workspace"),
        ...
        created = await deployment.http.handle(
            HTTPRequest(
                method="POST",
                path="/api/v1/tasks",
                headers=_headers(token, key="automatic-artifact-repair:task"),
                body={
                    "title": "Automatic Artifact repair Task",
                    "objective": "Produce an Artifact and repair it from independent review.",
                    "project_id": project_id,
                },
            )
        )
        ...
        await deployment.kernel.update_task(
            idempotency_key="automatic-artifact-repair:agent-binding",
            task_id=task_id,
        ...
        policy = deployment.verification.register_policy(
            VerificationPolicy(
                name="Automatic Artifact repair verification",
                stages=(
                    VerificationStage(
        ...
        queued = await deployment.http.handle(
            HTTPRequest(
                ...
                headers=_headers(token, key="automatic-artifact-repair:queue"),
            )
        )
        ...
        started = await deployment.http.handle(
            HTTPRequest(
                ...
                headers=_headers(token, key="automatic-artifact-repair:start"),
                body={
                    "workspace_id": workspace_id,
        ...
        original_run = await deployment.kernel.refresh_run(
            idempotency_key="automatic-artifact-repair:refresh",
            task_id=task_id,
        ...
        completed = await deployment.kernel.attach_artifact(
            idempotency_key="automatic-artifact-repair:artifact-v1",
            task_id=task_id,
            run_id=original_run_id,
        ...
        assert first_request.subject.subject_type == "artifact"
        assert second_request.subject.subject_type == "artifact"
        assert first_request.subject.subject_id == original_artifact_id
        assert second_request.subject.subject_id != original_artifact_id
        ...
        assert first_request.subject == first_result.subject
        assert second_request.subject == second_result.subject
        assert second_request.run_id is not None
        assert second_request.run_id != original_run_id

        first_file_id, first_artifact_id, first_payload = artifact_provider.outputs[0]
        second_file_id, second_artifact_id, second_payload = artifact_provider.outputs[1]
        assert first_artifact_id == first_request.subject.subject_id
        assert second_artifact_id == second_request.subject.subject_id
        assert first_file_id == first_request.subject.revision
        assert second_file_id == second_request.subject.revision
        ...
        assert second_artifact_id in repair_run.artifact_ids
        assert second_artifact_id not in original_run.artifact_ids

        canonical_task = await deployment.kernel.get_task(task_id)
        assert original_artifact_id in canonical_task.artifact_ids
        assert second_artifact_id in canonical_task.artifact_ids
        ...
        assert {record.artifact_ids[0] for record in producer_runs} == {
