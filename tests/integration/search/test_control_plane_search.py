...
from ai_multi_agent_platform.search import LocalSearchProvider, SearchDocument, SearchProvider
...
def test_global_search_exact_keyword_project_filter_and_run_lookup() -> None:
    async def scenario() -> None:
        ...
        queued = await http.handle(HTTPRequest(method="POST", path=f"/api/v1/tasks/{task_id}:queue", headers=api_headers(idempotency_key="search-queue")))
        assert queued.status == 200
        started = await http.handle(HTTPRequest(method="POST", path=f"/api/v1/tasks/{task_id}:start", headers=api_headers(idempotency_key="search-run")))
        assert started.status == 200
        assert isinstance(started.body, dict)
        run_id = started.body["id"]
        assert isinstance(run_id, str)
        keyword = await http.handle(HTTPRequest(method="GET", path="/api/v1/search", query={"q": "search"}))
        assert keyword.status == 200
        assert isinstance(keyword.body, dict)
        assert keyword.body["total"] >= 2
        items = keyword.body["items"]
        assert isinstance(items, list)
        identities = {(item["resource_type"], item["resource_id"]) for item in items if isinstance(item, dict)}
        assert ("project", project_id) in identities
        assert ("task", task_id) in identities
        for resource_type, resource_id in (("project", project_id), ("task", task_id), ("run", run_id)):
            exact = await http.handle(HTTPRequest(method="GET", path="/api/v1/search", query={"id": resource_id, "type": resource_type}))
        ...
        scoped = await http.handle(HTTPRequest(method="GET", path="/api/v1/search", query={"project_id": project_id, "type": "task,run"}))
        ...
        assert {item["resource_type"] for item in scoped_items if isinstance(item, dict)} == {"task", "run"}
        time_scoped = await http.handle(HTTPRequest(method="GET", path="/api/v1/search", query={"type": "task", "updated_after": "2000-01-01T00:00:00+00:00", "updated_before": "2100-01-01T00:00:00+00:00"}))
...
def test_search_authorization_filters_items_counts_and_snippets() -> None:
    async def scenario() -> None:
        ...
        visible_task = await _create_task(http, "visible-task", "Search visible task", visible_project)
        hidden_task = await _create_task(http, "hidden-task", "Search secret task", hidden_project)
        authorization.denied_project_id = hidden_project
        response = await http.handle(HTTPRequest(method="GET", path="/api/v1/search", query={"q": "Search"}))
...
def test_search_rebuild_removes_stale_provider_state_and_semantic_degrades_cleanly() -> None:
    async def scenario() -> None:
        ...
        await control_plane.search_provider.upsert(SearchDocument(resource_type="project", resource_id=stale_id, title="Stale search-only project", project_id=stale_id), OperationContext(correlation_id="stale-search-test"))
        stale = await http.handle(HTTPRequest(method="GET", path="/api/v1/search", query={"q": "Stale search-only"}))
        assert stale.status == 200
        assert isinstance(stale.body, dict)
        assert stale.body["total"] == 0
        canonical = await http.handle(HTTPRequest(method="GET", path="/api/v1/search", query={"id": project_id}))
        assert canonical.status == 200
        assert isinstance(canonical.body, dict)
        assert canonical.body["total"] == 1
        semantic = await http.handle(HTTPRequest(method="GET", path="/api/v1/search", query={"q": "meaning", "mode": "semantic"}))
...
def test_search_provider_unavailable_returns_retryable_503() -> None:
    async def scenario() -> None:
        _, http = _stack(FakeAuthorizationProvider(), search_provider=UnavailableSearchProvider())
        response = await http.handle(HTTPRequest(method="GET", path="/api/v1/search", query={"q": "search"}))
...
def test_search_time_filter_rejects_naive_timestamp() -> None:
    async def scenario() -> None:
        ...
        response = await http.handle(HTTPRequest(method="GET", path="/api/v1/search", query={"updated_after": "2026-09-03T10:00:00"}))
