...
def test_network_transport_preserves_nack_redelivery_and_dead_letter_semantics() -> None:
    async def scenario() -> None:
        broker = TcpMessageBroker(authentication_key=TEST_AUTHENTICATION_KEY)
        await broker.start()
        transport = TcpMessageTransport(
        ...
        try:
            ...
            await transport.nack(first, retry=True, reason="retry-once")
...
def test_network_transport_exposes_bounded_backpressure_without_silent_drop() -> None:
    async def scenario() -> None:
...
def test_graceful_client_shutdown_keeps_retained_work_available_for_reconnect() -> None:
    async def scenario() -> None:
        broker = TcpMessageBroker(authentication_key=TEST_AUTHENTICATION_KEY)
        await broker.start()
        first_transport = TcpMessageTransport(
            broker.host,
            broker.port,
            authentication_key=TEST_AUTHENTICATION_KEY,
        ...
        try:
            ...
            delivery = await asyncio.wait_for(anext(stream), timeout=2.0)
            assert delivery.envelope.message_id
            await replacement.ack(delivery)
            await stream.aclose()
        finally:
            await replacement.close(graceful=False)
            await broker.close(graceful=False)
