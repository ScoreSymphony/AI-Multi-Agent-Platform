...
def test_public_shell_mounts_governed_learning_routes() -> None:
    ...
    assert "commands={learningCapabilities.commands}" in routes
    assert "postPromotionAvailable={learningCapabilities.postPromotionAvailable}" in routes
...
def test_public_cli_registers_and_dispatches_learning_domain() -> None:
    ...
    assert 'requested_area not in {"registry", "marketplace", "learning", "trace"}' in cli
