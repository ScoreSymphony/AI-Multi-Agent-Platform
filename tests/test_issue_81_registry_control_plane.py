...
def test_enabled_registry_registers_discovery_but_not_activation() -> None:
    item, artifact = _item()
    ...
    assert REGISTRY_PREVIEW_COMMAND not in control_plane.commands
    assert "registry.activate" not in control_plane.commands
...
def test_preview_uses_server_resolved_validation_context() -> None:
    item, artifact = _item()
    distribution = DistributionService(
        LocalRegistryProvider((item,), {(item.item_id, item.version): artifact})
    )
