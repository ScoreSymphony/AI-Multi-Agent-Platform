...
def test_kernel_executes_end_to_end_through_reference_executor(tmp_path: Path) -> None:
    workspace_root = tmp_path / "workspaces"
    workspace = workspace_root / "task-demo"
    workspace.mkdir(parents=True)
    lifecycle = ExecutorLifecycleBackend(
        ReferenceExecutor(workspace_root), workspace="task-demo", action="echo"
    )
    kernel = PlatformKernel(
        orchestrator=FakeOrchestrator(), lifecycle=lifecycle, events=FakeEventProvider()
    )
    ...
    async def scenario() -> None:
        ...
        run = await kernel.start_task(command_id="cmd-start", task_id="task-demo")
