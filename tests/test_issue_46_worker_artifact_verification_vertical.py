...
def test_authenticated_worker_artifact_is_exact_verification_evidence_same_run(
    ...
    """Exercise the full #46 return path without a conformance-only File/Artifact shortcut."""
    ...
    async def scenario() -> None:
        ...
        transport = InProcessMessageTransport(provider_id="issue-46-worker-artifact-vertical")
        ...
        materializing_worker = MaterializingWorkerDispatcher(
            ExecutorWorker(
                worker_id,
            ...
            result_integrator=CanonicalWorkerArtifactIntegrator(
                deployment.files,
                deployment.kernel,
            ),
        )
        distributed.attach_worker(materializing_worker)
        ...
        try:
            ...
            assert isinstance(artifact_refs, tuple)
            ...
            assert isinstance(tool_refs, tuple)
            ...
            assert capability_result["artifact_refs"] == (artifact_id,)
            ...
            evidence = materializing_worker.evidence(worker_job_id)
            ...
            payload = b"".join(
                [chunk async for chunk in deployment.files.stream_file(change.file_id, file_context)]
            )
            ...
            assert len([record for record in distributed.records() if record.job.execution.run_id == run_id]) == 1
