...
def test_heterogeneous_placement_routes_and_rejects_canonically(tmp_path: Path) -> None:
    ...
    assert report.correctness.constraint_evidence_failures == 0
    ...
    assert report.rejection_code_counts["gpu_required"] >= 6
    assert report.rejection_code_counts["label_mismatch"] >= 4
    assert report.rejection_code_counts["network_unavailable"] >= 2
    assert report.rejection_code_counts["vram_insufficient"] >= 2
    assert report.constraint_evidence_counts == {
        "browser-network:cpu:network_unavailable": 2,
        "browser-network:gpu:label_mismatch": 2,
        "cpu-only:browser:label_mismatch": 2,
        "cpu-only:gpu:gpu_required": 2,
        "gpu-inference:browser:gpu_required": 2,
        "gpu-inference:cpu:gpu_required": 2,
        "unschedulable-vram:gpu:vram_insufficient": 2,
    }
