...
def test_plugin_control_plane_lifecycle_uses_registered_canonical_surfaces() -> None:
    async def scenario() -> None:
        ...
        for command in (
            ...
            assert command in manifest.body["commands"]

        before = await http.handle(
            HTTPRequest(method="GET", path="/api/v1/plugin-candidates", headers=_headers("read"))
        )
        assert before.status == 200
        assert isinstance(before.body, dict)
        assert before.body["total"] == 0
        ...
        listed = await http.handle(
            HTTPRequest(method="GET", path="/api/v1/plugins", headers=_headers("plugin-list"))
        )
        assert listed.status == 200
        assert isinstance(listed.body, dict)
        assert listed.body["total"] == 1
        item = listed.body["items"][0]
        assert isinstance(item, dict)
        assert item["id"] == reference_manifest().plugin_id
