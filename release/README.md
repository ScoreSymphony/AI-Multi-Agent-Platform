...
- The release-manifest workflow generates a deterministic SPDX 2.3 SBOM from the exact Python
  ...
  path to create signed build-provenance and SBOM attestations for the exact wheel. The signed
  bundles, SBOM and checksums are retained with the release evidence.
...
passed gates or approvals. The workflow resolves the reviewed SBOM/provenance placeholders only
from the signed-attestation URLs returned by GitHub and persists the resolved generation input as
release evidence.
