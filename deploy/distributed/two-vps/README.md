...
The canonical Node/Worker/scheduler path, message transport, service authentication, authorization
and distributed composition remain authoritative.

Repository-side topology, placeholder configuration and validation tooling are maintained here.
Real reference-host execution is retained as sanitized external evidence. Existing `issue562-*`
evidence/schema names remain stable historical evidence identifiers; they do not define current
runtime ownership or require the issue tracker to operate this topology.
...
That runbook defines both repository-side preparation and the separately retained real-host
execution evidence.
