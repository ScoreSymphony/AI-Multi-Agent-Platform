...
The normal single-node profile also ships a conservative starter Registry, so optional reviewed packages can appear in this component step without an operator first configuring `AI_MAP_REGISTRY_CATALOG`. The starter catalog currently exposes two real governed packages across distinct semantic kinds: the platform-owned **Hermes adapter** as an optional Orchestrator and the bundled **Reference echo capability provider** as a local/offline Capability Provider. Installing Hermes does not install or start the Hermes API server: the UI reports `adapter_installed` and keeps the separately managed self-hosted runtime as an explicit prerequisite. Installing the reference provider likewise does not silently enable code; configuration and activation remain explicit Plugin-owner actions, after which it exposes the deterministic `plugin.echo` capability. No remote or paid provider is selected implicitly. Categories for which the repository has no reviewed, lifecycle-safe distributable package remain honestly empty rather than receiving placeholders. An explicitly configured `registry_catalog` remains the operator-controlled replacement distribution source, while `AI_MAP_REGISTRY_ENABLED=false` preserves an explicit no-Registry deployment when desired.
